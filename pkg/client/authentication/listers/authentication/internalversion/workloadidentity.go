...
type WorkloadIdentityLister interface {
	...
	List(selector labels.Selector) (ret []*authentication.WorkloadIdentity, err error)
...
// List lists all WorkloadIdentities in the indexer.
func (s *workloadIdentityLister) List(selector labels.Selector) (ret []*authentication.WorkloadIdentity, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*authentication.WorkloadIdentity))
	})
	return ret, err
}
...
type WorkloadIdentityNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*authentication.WorkloadIdentity, error)
...
// List lists all WorkloadIdentities in the indexer for a given namespace.
func (s workloadIdentityNamespaceLister) List(selector labels.Selector) (ret []*authentication.WorkloadIdentity, err error) {
...
// Get retrieves the WorkloadIdentity from the indexer for a given namespace and name.
func (s workloadIdentityNamespaceLister) Get(name string) (*authentication.WorkloadIdentity, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(authentication.Resource("workloadidentity"), name)
	}
	return obj.(*authentication.WorkloadIdentity), nil
}
