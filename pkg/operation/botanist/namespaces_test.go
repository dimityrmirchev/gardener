...
var _ = Describe("Namespaces", func() {
	...
	Describe("#DeploySeedNamespace", func() {
		...
		It("should successfully deploy the namespace with enabled audit backend label", func() {
			auditBackendProviderType := "foo-audit-backend"
			defaultShootInfo.Spec.Kubernetes = gardencorev1beta1.Kubernetes{
				KubeAPIServer: &gardencorev1beta1.KubeAPIServerConfig{
					AuditConfig: &gardencorev1beta1.AuditConfig{
						Backend: &gardencorev1beta1.AuditBackend{
							Type: auditBackendProviderType,
						},
					},
				},
			}
			...
			Expect(botanist.SeedNamespaceObject).To(Equal(&corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					...
					Labels: map[string]string{
						"auditbackend.gardener.cloud/provider":        auditBackendProviderType,
						"gardener.cloud/role":                         "shoot",
						"seed.gardener.cloud/provider":                seedProviderType,
						"shoot.gardener.cloud/provider":               shootProviderType,
						"networking.shoot.gardener.cloud/provider":    networkingProviderType,
		...
		It("should successfully remove the audit backend label from the namespace when configuration is missing", func() {
			auditBackendProviderType := "foo-audit-backend"
			botanist.Shoot.SetInfo(defaultShootInfo)
